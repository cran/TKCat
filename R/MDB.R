...
count_records.MDB <- function(x, ...) {
  d <- dims(x, ...)
  d$records |> rlang::set_names(d$name)
}
...
c.MDB <- function(...) {
  ...
  if (any(duplicated(dbNames))) {
    stop("MDBs to combine cannot have the same names")
  }
  names(MDBs) <- dbNames
...
merge.MDB <- function(
  ...
  return(metaMDB(
    MDBs = list(x, y) |>
      rlang::set_names(c(db_info(x)$name, db_info(y)$name)),
...
#' @param jtName the name of the joint table. If NA (default), it is
...
join_mdb_tables <- function(
  ...
  toAdd$indexes <- list()
  toAdd <- list(toAdd) |> rlang::set_names(techname)
  ...
  ## metaMDB object ----
  MDBs <- list(rx) |> rlang::set_names(db_info(rx)$name)
  rt <- list(jt) |> rlang::set_names(techname)
  toRet <- metaMDB(
    MDBs = MDBs,
    relationalTables = rt,
    dataModel = jdm,
  ...
  toRet <- join_mdb_tables(
    ...
    dplyr::rename(dplyr::all_of(rlang::set_names(techname, jtName)))
  return(toRet)
}
